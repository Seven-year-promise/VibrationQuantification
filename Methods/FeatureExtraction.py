...
class SIFT:
    ...
    def detect_keypoints(self, ori_im):
        self.gray = cv2.cvtColor(ori_im, cv2.COLOR_BGR2GRAY)
        self.keypoints, self.descriptors = self.sift.detectAndCompute(self.gray, None)

        return self.keypoints, self.descriptors

    def drawpoints(self, im):
        im = cv2.drawKeypoints(self.gray, self.keypoints, im)
        plt.imshow(im)

        return im

    def compute_sift(self, image, sigma=1.6, num_intervals=3, assumed_blur=0.5, image_border_width=5):
...
def computeKeypointsWithOrientations(keypoint, octave_index, gaussian_image, radius_factor=3, num_bins=36, peak_ratio=0.8, scale_factor=1.5):
    ...
    orientation_peaks = np.where(np.logical_and(smooth_histogram > np.roll(smooth_histogram, 1), smooth_histogram > roll(smooth_histogram, -1)))[0]
